...
if (!("COOKIE" in process.env)) {
 throw new Error('We expected the COOKIE environment variable, but it was not set');
}
...
async function test(isExperiment) {
  ...
  return new Promise((resolve,reject) => {
    http.get('https://www.linkedin.com/feed/', {
      "credentials":"include",
      headers,
      ...
      res.on('end', x => {
        ...
        const hasFastlane = new jsdom.JSDOM(body).window.document.querySelector('meta[name=Fastlane]');

        if (isExperiment === true && hasFastlane === false) {
          throw new Error('expected Fastlane meta tag to be present when in the enabled cohort')
        }

        if (isExperiment === false && hasFastlane === true) {
          throw new Error('did NOT expected Fastlane meta tag to be present when in the control cohort')
        }
...
(async function main() {
  ...
  console.log(`Running: '${COUNT}' Samples (EXPERIMENT vs CONTROL)`);
  ...
  console.log(`contentDownload.avg delta(enabled - control): ${summarizedExperiment.contentDownload.avg - summarizedControl.contentDownload.avg}`);
